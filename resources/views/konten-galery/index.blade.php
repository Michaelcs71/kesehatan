...
@section('page-header')
    ...
    <div>
        <h4 class="fw-bold mb-1"><i class="ri ri-image-line text-primary me-1"></i> Galeri</h4>
        <small class="text-muted">Kelola foto galeri yang tampil di halaman publik.</small>
    </div>
@endsection
...
@php
    $stats = \App\Services\GaleriService::getStats();
@endphp
    ...
    <div class="col-6 col-md-4">
        <a href="javascript:void(0)" onclick="filterByStatus('')" class="text-decoration-none">
            <x-card class="card-hover h-100">
                <div class="text-muted small"><i class="ri ri-list-check"></i> Total Foto</div>
...
<x-card title="Daftar Foto Galeri" icon="ri-image-line">
    <x-slot:headerActions>
        @can('konten-galery.create')
            <a href="{{ route('konten-galery.create') }}" class="btn btn-primary btn-sm">
    ...
    <div class="row g-2 mb-3">
        <div class="col-md-8">
            <input type="text" id="searchInput" class="form-control form-control-sm" placeholder="Cari judul atau deskripsi...">
        </div>
        <div class="col-md-4">
        ...
        <table id="datatable" class="table table-hover w-100">
            <thead>
                <tr>
                    ...
                    <th class="text-center" style="width: 12%">Foto</th>
                    <th>Judul</th>
                    <th>Deskripsi</th>
...
window.whenKesehatanReady(function() {
    ...
    const CONFIG = {
        ROUTES: {
            DATA:    '{{ route('konten-galery.data') }}',
            SHOW:    '{{ route('konten-galery.show', ':id') }}',
            EDIT:    '{{ route('konten-galery.edit', ':id') }}',
            DESTROY: '{{ route('konten-galery.destroy', ':id') }}',
        },
        STORAGE_KEY: 'konten-galery',
    };
    ...
    new DataGrid({
        ...
        columns: [
            ...
            { data: 'gambar_path', orderable: false, className: 'text-center',
              render: (v) => v ? `<img src="/storage/${v}" style="width:54px;height:54px;object-fit:cover;border-radius:8px;">` : '<span class="text-muted">-</span>' },
            ...
            { data: 'deskripsi', orderable: false,
              render: (v) => v ? `<span class="text-muted small">${v.length > 60 ? v.substr(0, 60) + '...' : v}</span>` : '<span class="text-muted">-</span>' },
