...
@section('title', isset($id) ? 'Edit Artikel' : 'Tambah Artikel')

@section('page-header')
    <nav aria-label="breadcrumb" class="small">
        <ol class="breadcrumb mb-2">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="text-decoration-none">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="{{ route('konten-edukasi.index') }}" class="text-decoration-none">Edukasi</a></li>
            <li class="breadcrumb-item active">{{ isset($id) ? 'Edit' : 'Tambah' }}</li>
        </ol>
    </nav>
    <div>
        <h4 class="fw-bold mb-1" id="formTitle">{{ isset($id) ? 'Edit Artikel' : 'Tambah Artikel Edukasi' }}</h4>
        <small class="text-muted">Artikel yang dipublikasi akan tampil di halaman publik.</small>
    </div>
@endsection

@section('content')

<form id="edukasiForm" novalidate enctype="multipart/form-data">
    @csrf
    @if(isset($id))
        @method('PUT')
        <input type="hidden" id="edukasiId" value="{{ $id }}">
    @endif

    <div class="row g-4">
        <div class="col-lg-8">
            <x-card title="Informasi Artikel" icon="ri-book-line">
                <div class="mb-3">
                    <label for="judul" class="form-label form-label-required">Judul</label>
                    <input type="text" id="judul" name="judul" class="form-control" placeholder="Judul artikel" required maxlength="200">
                    <div class="invalid-feedback"></div>
                </div>

                <div class="mb-3">
                    <label for="kategori" class="form-label">Kategori</label>
                    <input type="text" id="kategori" name="kategori" class="form-control" placeholder="contoh: Diabetes, Nutrisi, Gaya Hidup" maxlength="100" list="kategoriList">
                    <datalist id="kategoriList">
                        <option value="Diabetes"></option>
                        <option value="Nutrisi"></option>
                        <option value="Gaya Hidup"></option>
                        <option value="Pengobatan"></option>
                        <option value="Pencegahan"></option>
                    </datalist>
                    <div class="invalid-feedback"></div>
                </div>
                ...
                <div class="mb-0">
                    <label for="konten" class="form-label form-label-required">Isi Artikel</label>
                    <textarea id="konten" name="konten" class="form-control" rows="12" placeholder="Tulis isi artikel di sini..." required></textarea>
                    <div class="invalid-feedback"></div>
                </div>
            </x-card>
        </div>
        ...
        <div class="col-lg-4">
            <x-card title="Gambar Sampul" icon="ri-image-line">
    ...
    <div class="d-flex gap-2 mt-4">
        ...
        <a href="{{ route('konten-edukasi.index') }}" class="btn btn-outline-secondary">Batal</a>
    </div>
</form>
...
window.whenKesehatanReady(function() {
    ...
    const CONFIG = {
        ...
        ROUTES: {
            INDEX:     '{{ route('konten-edukasi.index') }}',
            STORE:     '{{ route('konten-edukasi.store') }}',
            UPDATE:    '{{ isset($id) ? route('konten-edukasi.update', $id) : '' }}',
            SHOW_DATA: '{{ isset($id) ? route('konten-edukasi.show-data', $id) : '' }}',
        },
    };

    const $form = $('#edukasiForm');
