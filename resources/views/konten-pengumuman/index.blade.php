...
@section('page-header')
    ...
    <div>
        <h4 class="fw-bold mb-1"><i class="ri ri-megaphone-line text-primary me-1"></i> Pengumuman</h4>
        <small class="text-muted">Kelola pengumuman yang tampil di halaman publik.</small>
    </div>
@endsection
...
@php
    $stats = \App\Services\PengumumanService::getStats();
@endphp
    ...
    <div class="col-6 col-md-4">
        <a href="javascript:void(0)" onclick="filterByStatus('')" class="text-decoration-none">
            <x-card class="card-hover h-100">
                <div class="text-muted small"><i class="ri ri-list-check"></i> Total Pengumuman</div>
...
<x-card title="Daftar Pengumuman" icon="ri-megaphone-line">
    <x-slot:headerActions>
        @can('konten-pengumuman.create')
            <a href="{{ route('konten-pengumuman.create') }}" class="btn btn-primary btn-sm">
                <i class="ri ri-add-line me-1"></i> Tambah Pengumuman
            </a>
        @endcan
    </x-slot:headerActions>
    ...
    <div class="row g-2 mb-3">
        <div class="col-md-8">
            <input type="text" id="searchInput" class="form-control form-control-sm" placeholder="Cari judul atau isi...">
        </div>
        <div class="col-md-4">
...
window.whenKesehatanReady(function() {
    ...
    const CONFIG = {
        ROUTES: {
            DATA:    '{{ route('konten-pengumuman.data') }}',
            SHOW:    '{{ route('konten-pengumuman.show', ':id') }}',
            EDIT:    '{{ route('konten-pengumuman.edit', ':id') }}',
            DESTROY: '{{ route('konten-pengumuman.destroy', ':id') }}',
        },
        STORAGE_KEY: 'konten-pengumuman',
    };
    ...
    new DataGrid({
        ...
        columns: [
            ...
            { data: 'ringkasan', orderable: false,
